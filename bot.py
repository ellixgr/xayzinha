...
# File IDs dos seus vídeos (sem links externos!)
LISTA_VIDEOS_START = [
...
def formatar_tempo_restante(segundos):
    if segundos <= 0:
        return "Expirado"
    if segundos >= 315360000: # 10 anos = vitalício/permanente
        return "Permanente"
...
async def pegarid_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Só funciona para VOCÊ (dono): pega file_id de vídeo"""
...
async def clientes_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Lista clientes com valores e datas corretas"""
    if update.effective_user.id != DONO_ID:
        return
    agora = time.time()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Menu inicial com vídeos e planos"""
...
async def interceptador_universal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not usuario or usuario.id == DONO_ID:
        return
    # Só libera esses comandos para usuários comuns
...
async def verificar_saida_canal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Remove cliente do banco ao sair/ser expulso do canal"""
            ...
            if dados.split("_")[1] not in pagamentos_notificados:
                ...
                await context.bot.send_message(
                    ...
                    text=(
                        ...
                        f"Expira em: {'PERMANENTE' if nome_plano=='PERMANENTE/VITALÍCIO' else formatar_data_rj(data_expira)}"
                    )
                )
        else:
...
async def gerenciador_assinaturas(app):
    await asyncio.sleep(10) # Espera bot iniciar completamente
    while True:
        agora = time.time()
        for cliente in collection_clientes.find({}):
            ...
            uid = cliente["user_id"]
            # Aviso 1 dia antes
            if 82800 <= restante <= 86400 and not cliente.get("aviso_1dia"):
                try:
                    await app.bot.send_message(uid, "⚠️ SEU PLANO VENCE AMANHÃ! Garanta renovar agora para não perder o acesso!")
                    collection_clientes.update_one({"user_id":uid}, {"$set":{"aviso_1dia":True}})
                except Exception: pass
            # Aviso 20 minutos antes
            elif 0 < restante <= 1200 and not cliente.get("aviso_20min"):
                try:
                    await app.bot.send_message(uid, "🚨 ALERTA: SEU PLANO EXPIRA EM POUCOS MINUTOS! Renove AGORA!")
                    collection_clientes.update_one({"user_id":uid}, {"$set":{"aviso_20min":True}})
                except Exception: pass
            # Expirou → remover do grupo e do banco
            elif restante <= 0:
                try:
                    await app.bot.kick_chat_member(CANAL_ALVO_ID, uid)
                    await app.bot.unban_chat_member(CANAL_ALVO_ID, uid) # Deixa entrar se renovar depois
                except Exception: pass
